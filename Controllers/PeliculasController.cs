	...
	public class PeliculasController : ControllerBase
	{
		...
		public async Task<ActionResult<PeliculasIndexDTO>> Get()
		{
			...
			var proximosEstrenos = await context.Peliculas
				...
				.OrderBy(x => x.FechaEstreno).Take(top)
				.ToListAsync();
		...
		public async Task<ActionResult<List<PeliculaDTO>>> Filtrar([FromQuery] FiltroPeliculasDTO filtroPeliculasDTO)
		{
			...
			if (!string.IsNullOrEmpty(filtroPeliculasDTO.CampoOrdenar))
			{
				...
				{
					//Por si quieres que quede registrado el error
					//Poner en el constructor ILogger logger
					//LoggerCategory.LogError(ex.Message, ex);
				}
		...
		public async Task<ActionResult<PeliculaDetallesDTO>> Get(int id)
		{
			...
			var peliculaDetallesDTO = mapper.Map<PeliculaDetallesDTO>(pelicula);

			return peliculaDetallesDTO;
		}
		...
		public async Task<ActionResult> Post([FromForm] PeliculaCreacionDTO peliculaCreacionDTO)
		{
			...
			if (peliculaCreacionDTO.Poster != null)
			{
				...
				using (var memoryStream = new MemoryStream())
				{
					...
					pelicula.Poster = await almacenadorArchivos.GuardarArhivo(contenido, extension, contenedor, peliculaCreacionDTO.Poster.ContentType);
				}				
			}
		...
		public async Task<ActionResult> Put([FromForm] PeliculaCreacionDTO peliculaCreacionDTO, int id)
		{
			...
			if (peliculaCreacionDTO.Poster != null)
			{
				...
				using (var memoryStream = new MemoryStream())
				{
					...
					var contenido = memoryStream.ToArray();
					var extension = Path.GetExtension(peliculaCreacionDTO.Poster.FileName);
					peliculaDB.Poster = await almacenadorArchivos.EditarArhivo(contenido, extension, contenedor, peliculaDB.Poster, peliculaCreacionDTO.Poster.ContentType);
				}
			}
